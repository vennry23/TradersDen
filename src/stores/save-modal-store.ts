...
export default class SaveModalStore implements ISaveModalStore {
    ...
    async onConfirmSave({ is_local, save_as_collection, bot_name }: IOnConfirmProps): Promise<void> {
        ...
        if (active_tab === 1) {
            xml = Blockly?.Xml?.workspaceToDom(Blockly?.derivWorkspace) as Element;
        } else {
            ...
            if (main_strategy) {
                ...
                xml = Blockly.utils.xml.textToDom(main_strategy.xml) as Element;
            }
        }
    ...
    private async addStrategyToWorkspace(
        workspace_id: string,
        is_local: boolean,
        save_as_collection: boolean,
        bot_name: string,
        ...
        try {
            ...
            const save_type = load_modal.getSaveType(type)?.toLowerCase();
            ...
            workspace
                .sort((a: TStrategy, b: TStrategy) => b.timestamp - a.timestamp)
                .slice(0, MAX_STRATEGIES);

            const { setRecentStrategies } = load_modal;
            await localForage.setItem('saved_workspaces', LZString.compress(JSON.stringify(workspace)));
